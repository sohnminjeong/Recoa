								...
								<li class="page-item ${paging.prev ? '' : 'disabled'}">
									...
									<c:choose>
										<c:when
											...
											<a class="page-link"
												href="/allGuest?select=${paging.select}&keyword=${paging.keyword}&page=${paging.startPage=1}">Previous</a>
										</c:when>
										<c:when
											test="${(paging.startPage == 1)&&(paging.select == null) && (paging.keyword == null)}">
											<a class="page-link" href="/allGuest?page=${paging.startPage=1}"><</a>
										</c:when>
										<c:otherwise>
											<a class="page-link" href="/allGuest?page=${paging.startPage-1}"><</a>
										</c:otherwise>
										...
										<c:choose>
											<c:when test="${(paging.select != null) && (paging.keyword != null)}">
												<a class="page-link ${paging.page== page ? 'active' : ''}"
													href="/allGuest?select=${paging.select}&keyword=${paging.keyword}&page=${page}"
													id="page_num">
													${page}
												</a>
											</c:when>
											<c:otherwise>
												<a class="page-link ${paging.page== page ? 'active' : ''}"
													href="/allGuest?page=${page}" id="page_num">
													${page}
												</a>
											</c:otherwise>
										</c:choose>
								...
								<li class="page-item ${paging.next ? '' : 'disabled'}">
									<c:choose>
										<c:when
											...
											<a class="page-link"
												href="/allGuest?select=${paging.select}&keyword=${paging.keyword}&page=${paging.endPage=paging.endPage}">Next</a>
										</c:when>
										<c:when
											test="${(paging.endPage < 10)&&(paging.select == null) && (paging.keyword == null)}">
											<a class="page-link"
												href="/allGuest?page=${paging.endPage=paging.endPage}">></a>
										</c:when>
										<c:otherwise>
											<a class="page-link" href="/allGuest?page=${paging.endPage + 1}">></a>
										</c:otherwise>
									</c:choose>
								</li>
							</ul>
